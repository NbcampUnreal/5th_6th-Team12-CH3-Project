...
void AMyGameState::BeginPlay()
{
	...
	//	Portals[i]->SetActorHiddenInGame(true);     // 화면에서 안 보이게
	//	Portals[i]->SetActorEnableCollision(false); // 충돌 비활성화
	//	Portals[i]->SetActorTickEnabled(false);
	//}	
}
...
void AMyGameState::Tick(float DeltaTime)
{
	...
	if (GetWorld()->GetFirstPlayerController()->WasInputKeyJustPressed(EKeys::K)) { PortalsOpen(); }
...
void AMyGameState::PortalsOpen()
{
	for (int i = 0; i < Portals.Num(); i++) {
		Portals[i]->SetActorHiddenInGame(false);     // 화면에서 안 보이게
		Portals[i]->SetActorEnableCollision(true); // 충돌 비활성화
		Portals[i]->SetActorTickEnabled(true);
	}
}
