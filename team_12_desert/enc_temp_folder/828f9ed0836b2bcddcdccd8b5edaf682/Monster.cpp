...
AMonster::AMonster()
{
	...
	// OverheadWidget->SetRelativeTransform(FTransform3d(0, 0, 100));
}
...
void AMonster::BeginPlay()
{
	...
	if (OverheadWidget) {
		...
		if (Widget) {
			...
			//Widget->GetWidgetFromName(TEXT("HpBar"))
			if (temp) {
				UE_LOG(LogTemp, Warning, TEXT("bad2"));
			}
...
void AMonster::HpBarProgress()
{
	if (HPBar)
	{
		...
		HPBar->SetPercent(0.3f);
		UE_LOG(LogTemp, Warning, TEXT("bad3=%f"), HPBar->GetPercent());
	}
}
